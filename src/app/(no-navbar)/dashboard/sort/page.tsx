    ...
    >
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-500">Parent: {page.parentId || "None"}</span>
        </div>
      </div>
    </div>
  )
...
const ArticleList = () => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activePage.level !== overPage.level || activePage.parentId !== overPage.parentId) {
    ...
    const siblings = flattenedPages.filter(
      (page) => page.level === activePage.level && page.parentId === activePage.parentId,
    )
    ...
    <div className="article-list-container container mx-auto px-4 py-6 max-w-4xl">
      ...
      {flattenedPages.length === 0 ? (
        ...
        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={flattenedPages.map((page) => page.id)} strategy={verticalListSortingStrategy}>
