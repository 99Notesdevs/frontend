...
export default function EditTestSeries() {
  ...
  const fetchTestSeries = async () => {
    try {
      setIsLoading(true);
      const response = (await api.get(`/testSeries/${params.id}`)) as {
        data: TestSeriesData;
      };
      const { data } = response;
  ...
  const fetchAvailableQuestions = async (categoryId: number) => {
    try {
      ...
      const response = (await api.get(
        `/questions?categoryId=${categoryId}&limit=${pageSize}`
      )) as { data: any[] };
      const { data } = response;
  ...
  const handleQuestionRemove = (questionId: number) => {
    setSelectedQuestionIds((prev) => {
      const currentIds = prev || [];
      const newIds = Array.isArray(currentIds)
  ...
  const handleSubmit = async (data: TestSeriesData) => {
    try {
      const response = (await api.put(`/testSeries/${params.id}`, {
        name: data.name,
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e0e7ef] py-10 px-2 md:px-6">
      <div className="container mx-auto max-w-7xl">
        <div className="flex items-center mb-8">
          <button
            onClick={() => router.push("/dashboard/testSeries")}
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
              ...
              <p className="text-gray-600 mb-6">
                Choose a category to view and select questions for your test
                series.
              </p>
              ...
              {availableQuestions.length > 0 ? (
                ...
                <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Select a category to view available questions
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
              ...
              {selectedQuestionIds.length > 0 ? (
                ...
                <div className="text-center py-8 bg-gray-50 rounded-lg">
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    Select questions from the list above to add them to your
                    test series.
                  </p>
                </div>
              )}
